...
public class ItemPriceTooltip(PriceInsightPlugin plugin) : IDisposable {
    ...
    private List<Payload> ParseMbData(bool hq, MarketBoardData? mbData, LookupState lookupState) {
        ...
        if (lookupState == LookupState.Faulted) {
            ...
            payloads.Add(new TextPayload(" 無法取得市場佈告板資訊。\n        Universalis API 可能發生問題。\n        請稍候，或查看 Universalis Discord。\n        按 Alt 重試，或使用 /xllog 查看日誌。"));
            payloads.Add(new UIForegroundPayload(0));
        } else if (mbData == null) {
            payloads.Add(new UIForegroundPayload(20));
            payloads.Add(new IconPayload(BitmapFontIcon.LevelSync));
            payloads.Add(new TextPayload(" 正在取得市場佈告板資訊……"));
            payloads.Add(new UIForegroundPayload(0));
        } else {
            ...
            void PriceHeader() {
                ...
                payloads.Add(new TextPayload("市場佈告板價格："));
                priceHeader = true;
            }
            ...
            void RecentHeader() {
                ...
                if (payloads.Count > 0)
                    payloads.Add(new TextPayload("\n"));
                payloads.Add(new TextPayload("最近成交："));
                recentHeader = true;
            }
            ...
            if (plugin.Configuration.ShowDailySaleVelocityIn > 0) {
                ...
                if (saleVelocity != null && GetNqHqData(saleVelocity.Nq, saleVelocity.Hq) != null) {
                    if (payloads.Count > 0)
                        payloads.Add(new TextPayload("\n"));
                    payloads.Add(new TextPayload($"每日成交量（{scope}）："));
            ...
            if (payloads.Count == 0) {
                payloads.Add(new UIForegroundPayload(20));
                payloads.Add(new TextPayload("沒有此物品的市場佈告板資訊。\n請嘗試開啟遊戲內的市場佈告板。"));
                payloads.Add(new UIForegroundPayload(0));
            }
        }
